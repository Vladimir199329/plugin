 ...
 */
class SeSearch extends SeAbstractExtension
{
    // Request errors
    ...
    public function preGetPosts(&$query)
    {
        if (is_search() && is_shop()) {
            $this->search_processed = $this->executeSearchRequest($query->query, $query->query_vars, self::TYPE_TEXT_FIND, $this->lang_code);
        }
    }
    ...
    private function executeSearchRequest($query, $query_vars, $type, $lang_code)
    {
        ...
        if ($type == self::TYPE_TEXT_FIND && (!isset($params['q']) || $params['q'] == '')) {
            return false;
        }
    ...
    private function sendSearchAndRequest(array $params, $lang_code)
    {
        ...
        if (empty($result) || $this->getIsUseRequestCache() != true) {
            ...
            SeLogger::getInstance()->debug(SE_SERVICE_URL . '/search?api_key=' . $api_key . '&' . $query);
            ...
            if (strlen($query) > SE_MAX_SEARCH_REQUEST_LENGTH) {
                ...
                if (!is_wp_error($result)) {
                    ...
                    SeLogger::getInstance()->debug(sprintf(__('Error occurs during request %s'), $request_error));
                }
            ...
            SeLogger::getInstance()->debug($result);
            ...
            if (isset($result['error'])) {
                ...
                SeLogger::getInstance()->debug($result['error']);
...
if (!is_admin()) {
    add_action('plugins_loaded', function() {
        $GLOBALS['SearchaniseSearch'] = new SeSearch();
    });
}
